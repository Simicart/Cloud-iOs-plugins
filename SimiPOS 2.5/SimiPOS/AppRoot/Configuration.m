...
+ (RPCache *)productCache {
    static RPCache *productCache = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        productCache = [[self alloc] init];
    });
    return productCache;
}
// end
...
- (id)init
{
    if (self = [super init]) {
        ...
        // lionel added for cache product
        self.productCache = [[RPCache alloc] init];
        self.productCacheResult = [[NSCache alloc] init];
        // end
