...
- (void)refineSubAccount
{
    ...
    if ([storeUrls objectForKey:@"lives"] && [[storeUrls objectForKey:@"lives"] isKindOfClass:[NSArray class]]) {
        for (NSString *url in [storeUrls objectForKey:@"lives"]) {
            NSString *refinedUrl = [[self class] refineSimiPOSURL:url];
            if (refinedUrl) {
                [urls addObject:@{
    ...
    if ([storeUrls objectForKey:@"devs"] && [[storeUrls objectForKey:@"devs"] isKindOfClass:[NSArray class]]) {
        for (NSString *url in [storeUrls objectForKey:@"devs"]) {
            NSString *refineUrl = [[self class] refineSimiPOSURL:url];
            if (refineUrl) {
                [urls addObject:@{
                    @"store_url": refineUrl,
                    @"store_plan": @"dev"
...
- (void)refineMainAccount
{
    ...
    [terms enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        // Live domain
        NSString *liveUrl = [[self class] refineSimiPOSURL:[obj objectForKey:@"live_domain"]];
        ...
        // Dev domain
        NSString *devUrl = [[self class] refineSimiPOSURL:[obj objectForKey:@"dev_domain"]];
...
+ (NSString *)refineSimiPOSURL:(NSString *)storeUrl
{
    ...
    return [storeUrl stringByAppendingString:@"/simipos"];
}
