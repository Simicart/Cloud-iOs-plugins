...
-(id)init
{
    if (self = [super init]) {
        ...
        if ([Configuration globalConfig].configPrice) {
            self.isLoadedFormat = YES;
            [self addEntriesFromDictionary:[Configuration globalConfig].configPrice];
        }else {
...
//        [self load:nil];
        DLog(@"Check config.setStoreData ---------- ----------- k tra ve config price");
    ...
    if (iPrice) {
        ...
        while (iPrice) {
            if (i == groupLength) {
                i = 0;
                if ([[self objectForKey:@"groupSymbol"] isEqualToString:@","] || [[self objectForKey:@"groupSymbol"] isEqualToString:@"."]) {
