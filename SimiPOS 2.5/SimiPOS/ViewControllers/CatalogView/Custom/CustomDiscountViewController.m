...
- (void)viewDidLoad
{
    ...
    // Navigation Title
    self.title = NSLocalizedString(@"Discount", nil);
...
- (IBAction)toggleInputType:(id)sender
{
    if ([inputType selectedSegmentIndex]) {
        [self.couponCode becomeFirstResponder];
        centerView.hidden = YES;
        couponView.hidden = NO;
    } else {
        [self.discountName becomeFirstResponder];
        couponView.hidden = YES;
...
- (void)toggleDiscountType:(id)sender
{
    if ([discountType selectedSegmentIndex]) {
        ...
        amountLabel.text = NSLocalizedString(@"Percentage", nil);
        discountPercentage.text = [NSString stringWithFormat:@"%.2f%%", percentage];
        keyboard.maxInput = 5;
        keyboard.currentValue = percentage;
        keyboard.floatPoints = 2;
...
- (void)addCustomDiscount:(id)sender
{
    ...
    [[[NSThread alloc] initWithTarget:self selector:@selector(addCustomDiscountThread) object:nil] start];
}

- (void)addCustomDiscountThread
{
    //Ravi
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didSetDiscount:) name:@"DidSetDisCount" object:nil];
    DiscountModel *discountModel = [DiscountModel new];
    NSMutableDictionary *dataDiscount = [NSMutableDictionary new];
    
    permission =[Permission MR_findFirst];

    if ([inputType selectedSegmentIndex] || (!permission.cart_custom_discount.boolValue && permission.cart_coupon.boolValue)) {
        // set coupon code
        [dataDiscount setValue:couponCode.text forKey:@"coupon"];
    } else {
        // set custom discount
        [dataDiscount setValue:[NSNumber numberWithDouble:keyboard.currentValue] forKey:@"amount"];
        [dataDiscount setValue:[NSNumber numberWithInteger:[discountType selectedSegmentIndex]] forKey:@"type"];
        if (discountName.text != nil) {
            [dataDiscount setValue:discountName.text forKey:@"description"];
        }
    }
    [discountModel setDiscountWithData:dataDiscount];
    ...
    id success = [[NSNotificationCenter defaultCenter] addObserverForName:@"DiscountAddSuccess" object:nil queue:nil usingBlock:^(NSNotification *note) {
        // Dismiss Popup
        [animation stopAnimating];
    ...
    if ([inputType selectedSegmentIndex] || (!permission.cart_custom_discount.boolValue && permission.cart_coupon.boolValue)) {
        // set coupon code
        [discount addCouponCode:couponCode.text];
    } else {
        // set custom discount
        [discount setValue:[NSNumber numberWithDouble:keyboard.currentValue] forKey:@"amount"];
        [discount setValue:[NSNumber numberWithInteger:[discountType selectedSegmentIndex]] forKey:@"type"];
        if (discountName.text != nil) {
            [discount setValue:discountName.text forKey:@"description"];
        }
...
//Ravi
- (void)didSetDiscount: (NSNotification*)noti{
    [animation stopAnimating];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        DLog(@"didSetDiscount - %@",respone.data);
        [self performSelectorOnMainThread:@selector(cancelEdit) withObject:nil waitUntilDone:NO];
