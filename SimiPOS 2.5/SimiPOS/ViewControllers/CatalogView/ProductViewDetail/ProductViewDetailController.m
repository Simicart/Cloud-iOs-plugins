...
- (void)viewDidLoad
{
    ...
    //Ravi
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(closeDetailPage) name:@"CloseDetailPage" object:nil];
    //End
}
...
- (void)loadViewProductDetail
{
    [animation startAnimating];
    //Ravi
    self.productFullInfo = [Product new];
    [self.productFullInfo addEntriesFromDictionary:self.product];
    if ([self.product objectForKey:@"detail"]) {
        Product *detailProduct = [Product new];
        NSMutableDictionary *detailData = [self.product objectForKey:@"detail"];
        [detailProduct addEntriesFromDictionary:detailData];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didGetProductDetail:) name:@"DidGetProductDetail" object:productDetailModel];
    [productDetailModel getProductDetail:[self.product getId]];
    // End
}

// Johan
- (void) didGetProductDetail:(NSNotification *) noti{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidGetProductDetail" object:productDetailModel];
    RetailerPosResponder *respone = [noti.userInfo valueForKey:@"responder"];
    [animation stopAnimating];
    if([respone.status isEqualToString:@"SUCCESS"]){
        Product *detailProduct = [Product new];
        NSMutableDictionary *returnData = [productDetailModel valueForKey:@"data"];
        [detailProduct addEntriesFromDictionary:returnData];
        
        if (![detailProduct getId]) {
            return;
        }
        self.product = detailProduct;
        [self.productFullInfo setObject:detailProduct forKey:@"detail"];
        [btnLoadDetail setHidden:YES];
        [self performSelectorOnMainThread:@selector(viewProductDetail) withObject:nil waitUntilDone:NO];
    }else{
        if (btnLoadDetail) {
            [btnLoadDetail setHidden:NO];
        }else{
            btnLoadDetail = [MSBlueButton buttonWithType:UIButtonTypeRoundedRect];
            btnLoadDetail.frame = CGRectMake(468, 50, 280, 44);
            [btnLoadDetail setTitle:NSLocalizedString(@"Load Detail Product", nil) forState:UIControlStateNormal];
            [self.detailView addSubview:btnLoadDetail];
            [self.detailView setContentSize:CGSizeMake(748, 100)];
            animation.frame = CGRectMake(0, 0, WINDOW_HEIGHT, 100);
            [detailView addSubview:animation];
            [btnLoadDetail addTarget:self action:@selector(loadViewProductDetail) forControlEvents:UIControlEventTouchUpInside];
        }
...
- (void)viewProductDetail
{
    ...
    UIView *separator = [[UIView alloc] initWithFrame:CGRectMake(0, 27, self.detailView.frame.size.width, 1)];
    ...
    if ([self.product objectForKey:@"short_description"]) {
        ...
        UITextView *overView = [[UITextView alloc] initWithFrame:CGRectMake(461, height, self.detailView.frame.size.width - 448 - 20, 588 - height)];
    ...
    if ([self.product hasOptions] && isAvailable) {
        ...
        productOptions.product = self.productFullInfo;
    ...
    if ([self.product objectForKey:@"description"]) {
        UILabel *descriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, height, self.detailView.frame.size.width - 20, 28)];
        ...
        UITextView *description = [[UITextView alloc] initWithFrame:CGRectMake(5, height, self.detailView.frame.size.width - 10, 24)];
    ...
    if ([self.product objectForKey:@"inventory"] && [[self.product objectForKey:@"inventory"] isKindOfClass:[NSArray class]] && [[self.product objectForKey:@"inventory"] count]) {
        UILabel *inventoryLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, height, width, 28)];
        inventoryLabel.font = [UIFont boldSystemFontOfSize:20];
        inventoryLabel.text = NSLocalizedString(@"Inventory", nil);
        inventoryLabel.textColor = [UIColor barBackgroundColor];
        [self.detailView addSubview:inventoryLabel];
        [inventoryLabel addSubview:[separator clone]];
        height += 33;
        
        for (NSDictionary *inventory in [self.product objectForKey:@"inventory"]) {
            UITextView *addLabel = [[UITextView alloc] initWithFrame:CGRectMake(10, height, 224, 24)];
            addLabel.text = [inventory objectForKey:@"name"];
            ...
            addValue.text = [NSString stringWithFormat:@"%@",[inventory objectForKey:@"qty"]];
    ...
    self.detailView.contentSize = CGSizeMake(width, height);
}
