...
- (void)viewDidLoad
{
    ...
    //Ravi fix bug hold order
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkProccessHoldOrder) name:QuoteEndRequestNotification object:nil];
    //End
    
    //Ravi fix bug add product error
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(queryException) name:@"QueryException" object:nil];
    //End
    
    //Ravi fix bug khong refresh lai cartinfomation khi sang trang checkout.
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshCartView) name:@"QuoteLoadAfter" object:nil];
    //End
...
-(void)checkProccessHoldOrder{
    if(self.isHoldOrder){
        [self.holdButton setTitle:NSLocalizedString(@"Cancel", nil) forState:UIControlStateNormal];
        self.holdCancelImage.image =[UIImage imageNamed:@"cancel_filled.png"];
        
    }else{
        [self.holdButton setTitle:NSLocalizedString(@"Hold", nil) forState:UIControlStateNormal];
...
-(void)cancelOnHoldOrder{
   ...
   // DLog(@"order:%@",self.order);
    ...
    if (![self.order objectForKey:@"increment_id"]) {
        [self.order setValue:[self.order getId] forKey:@"increment_id"];
    }
    orderId = [self.order objectForKey:@"increment_id"];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didCancelHoldOrder:) name:@"DidCancelHoldOrder" object:nil];
    CancelOnHoldOrderModel *cancelHoldOrderModel = [CancelOnHoldOrderModel new];
    [cancelHoldOrderModel cancelHoldOrderWithID:orderId];
...
-(void)holdOrderCart{
   ...
   // DLog(@"orderComment:%@ ; cashIn:%@",orderComment,cashIn);
    ...
    //Ravi
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didHoldOrder:) name:@"DidHoldOrder" object:nil];
    HoldOrderModel *holdOrderModel = [HoldOrderModel new];
    [holdOrderModel holdOrderWithCashIn:cashIn note:orderComment];
...
//Ravi
- (void)didHoldOrder : (NSNotification *)noti{
    [cartController stopAnimation];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        DLog(@"didHoldOrder - %@",respone.data);
        [Utilities toastSuccessTitle:@"Hold Order" withMessage:MESSAGE_SUBMIT_SUCCESS withView:self.view];
        [[Quote sharedQuote] cleanData];
        [self refreshCartView];
    }else{
        NSString * message = [respone.message objectAtIndex:0];
        if(message){
            [Utilities alert:@"Hold Order" withMessage:message];
        }else{
            [Utilities alert:@"Hold Order" withMessage:MESSAGE_SUBMIT_FAIL];
...
- (void)didCancelHoldOrder : (NSNotification *)noti{
    ...
    RetailerPosResponder *respone = [noti.userInfo valueForKey:@"responder"];
    if([respone.status isEqualToString:@"SUCCESS"]){
        DLog(@"didCancelHoldOrder - %@",respone.data);
        [Utilities toastSuccessTitle:@"Hold Order" withMessage:MESSAGE_CANCEL_SUCCESS withView:self.view];
        self.isHoldOrder =NO;
        self.order =nil;
        [self clearShoppingCart];
        ...
        NSString * message = [respone.message objectAtIndex:0];
        if(message){
            [Utilities alert:@"Cancel Hold Order" withMessage:message];
        }else{
            [Utilities alert:@"Cancel Hold Order" withMessage:MESSAGE_CANCEL_FAIL];
        }
    }
}
//End
