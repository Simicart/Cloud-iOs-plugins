...
- (void)viewDidLoad {
    ...
    for (NSString *key in [[self.order objectForKey:@"items"] allKeys]) {
        ...
        [item addEntriesFromDictionary:[[self.order objectForKey:@"items"] objectForKey:key]];
...
-(void)shipButtonClick{
    ...
    // Johan
    shipOrderModel = [ShipOrderModel new];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didShipOrder:) name:@"DidShipOrder" object:shipOrderModel];
    [shipOrderModel shipOrder:[self.order getIncrementId] WithItems:items];
    // End
}

// Johan
- (void) didShipOrder:(NSNotification *) noti{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidShipOrder" object:shipOrderModel];
