...
@implementation LoginFormViewController{
    ...
    LoginTryDemoModel *loginTryDemoModel;
    LoginWithCustomerModel *loginWithCustomerModel;
    // End
}
...
- (void)viewDidLoad
{
    ...
    if (![[Reachability reachabilityForInternetConnection] isReachable]) {
        [Utilities alert:NSLocalizedString(@"Network Error", nil) withMessage:NSLocalizedString(@"You must connect to a Wi-Fi or cellular data network to access the Retailer POS", nil)];
...
-(void)checkLoginMageStore{
    ...
    // Johan
    loginWithCustomerModel = [LoginWithCustomerModel new];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didLoginWithCustomer:) name:@"DidLoginWithCustomer" object:loginWithCustomerModel];
    [loginWithCustomerModel loginWithCustomer:username.text password:password.text];
    // End
    
}

// Johan
-(void) didLoginWithCustomer:(NSNotification *) noti {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidLoginWithCustomer" object:loginWithCustomerModel];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        if ([[NSUserDefaults standardUserDefaults] valueForKey:@"domain"] != nil) {
            if (![[[NSUserDefaults standardUserDefaults] valueForKey:@"domain"] isEqualToString:[[Configuration globalConfig] valueForKey:API_URL_NAME]]) {
                [[NSUserDefaults standardUserDefaults] setValue:@"1" forKey:@"change_domain"];
            } else {
                [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"change_domain"];
            }
        } else {
            [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"change_domain"];
        }
        [[NSUserDefaults standardUserDefaults] setValue:[[Configuration globalConfig] valueForKey:API_URL_NAME] forKey:@"domain"];
        //  DLog(@"result:%@",result);
        
        NSDictionary * data =[loginWithCustomerModel objectForKey:@"data"];
        ...
        //Ravi
        [[Configuration globalConfig] setObject:@"NO" forKey:@"trydemo"];
        //End
...
-(void)updateUserInfo:(NSDictionary *)dict{
    ...
    if(userInfoDict){
        ...
        // Johan: Check UserID when login
        NSString *currentUserID = [[NSUserDefaults standardUserDefaults] stringForKey:@"CurrentUserID"];
        if(currentUserID != nil && userInfo.user_id != nil){
            if ([currentUserID isEqualToString:userInfo.user_id]) {
                [[NSUserDefaults standardUserDefaults] setValue:@"1" forKey:@"checkUserID"];
            }else {
                [[NSUserDefaults standardUserDefaults] setValue:[userInfoDict objectForKey:@"user_id"] forKey:@"CurrentUserID"];
                [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"checkUserID"];
            }
        }else{
            [[NSUserDefaults standardUserDefaults] setValue:[userInfoDict objectForKey:@"user_id"] forKey:@"CurrentUserID"];
            [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"checkUserID"];
...
-(void)checkActiveKeyStatus{
    ...
    if(urlDomainConfig != nil && urlDomainConfig.api_key){
        ...
        [checkKeyModel checkKeyFromMagestore:urlDomainConfig.api_key];
    }
    // End
}
...
-(void)loginTryDemo{
    ...
    // Johan
    loginTryDemoModel = [LoginTryDemoModel new];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didLoginTryDemo:) name:@"DidLoginTryDemo" object:loginTryDemoModel];
    [loginTryDemoModel loginTryDemo];
    // End
}

// Johan
-(void) didLoginTryDemo:(NSNotification *) noti {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidLoginTryDemo" object:loginTryDemoModel];
    RetailerPosResponder *respone = [noti.userInfo valueForKey:@"responder"];
    [self enableSelfView];
    [self.loginBtnAnimation stopAnimating];
    if([respone.status isEqualToString:@"SUCCESS"]){
        NSDictionary * data =[loginTryDemoModel objectForKey:@"data"];
        
        if(data && [data isKindOfClass:[NSDictionary class]]){
            NSString * demo_url =[NSString stringWithFormat:@"%@",[data objectForKey:@"demo_url"]];
//            NSString * demo_url = @"http://dev-extension.magestore.com/everest/magento19/index.php/webpos/api";
            [[Configuration globalConfig] setObject:demo_url forKey:API_URL_NAME];
            if ([[NSUserDefaults standardUserDefaults] valueForKey:@"domain"] != nil) {
                if (![[[NSUserDefaults standardUserDefaults] valueForKey:@"domain"] isEqualToString: demo_url]) {
                    [[NSUserDefaults standardUserDefaults] setValue:@"1" forKey:@"change_domain"];
                } else {
                    [[NSUserDefaults standardUserDefaults] setValue:@"0" forKey:@"change_domain"];
                }
            ...
            [[NSUserDefaults standardUserDefaults] setValue:demo_url forKey:@"domain"];
...
-(void)goToTryDemo:(NSString *)userDemo Password:(NSString *)passwordDemo{
    ...
    // Johan
    loginWithCustomerModel = [LoginWithCustomerModel new];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didLoginWithCustomerDemo:) name:@"DidLoginWithCustomer" object:loginWithCustomerModel];
    [loginWithCustomerModel loginWithCustomer:userDemo password:passwordDemo];
    // End
    
}

// Johan
-(void) didLoginWithCustomerDemo:(NSNotification *) noti {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidLoginWithCustomer" object:loginWithCustomerModel];
    RetailerPosResponder *respone = [noti.userInfo valueForKey:@"responder"];
    [self.loginBtnAnimation stopAnimating];
    if([respone.status isEqualToString:@"SUCCESS"]){
        ...
        //Ravi
        [[Configuration globalConfig] setObject:@"YES" forKey:@"trydemo"];
        //End
