...
- (void)viewDidLoad
{
    ...
    UIButton *submitButton = [MSGrayButton buttonWithType:UIButtonTypeRoundedRect];
...
- (void)submitSignatureThread
{
    ...
    //Ravi
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didAddSignatureInvoice:) name:@"DidAddSignatureInvoiceWith" object:nil];
    SignatureInvoiceModel *signatureModel = [SignatureInvoiceModel new];
    [signatureModel addSignatureInvoiceWith:[[Quote sharedQuote].order objectForKey:@"invoice"] signatureData:signImageData];
    return;
    //End
...
- (void)didAddSignatureInvoice : (NSNotification*)noti{
    [animation stopAnimating];
    [animation removeFromSuperview];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        DLog(@"didAddSignatureInvoice - %@",respone.data);
        [self willMoveToParentViewController:nil];
        [self.view removeFromSuperview];
        [self removeFromParentViewController];
    }else{
