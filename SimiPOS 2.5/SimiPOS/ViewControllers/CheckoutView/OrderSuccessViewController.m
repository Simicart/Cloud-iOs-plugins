...
- (void)viewDidLoad
{
    ...
    // Success Information
    UILabel *totalLabel = [[UILabel alloc] initWithFrame:CGRectMake(98, 36, 400, 48)];
    totalLabel.text = [Price format:[[Quote sharedQuote] getGrandTotal]];
    ...
    orderSuccessImage.frame = CGRectMake(84, 116, 64, 64);
    [self.view addSubview:orderSuccessImage];
    
    UILabel *orderSuccessLabel = [[UILabel alloc] initWithFrame:CGRectMake(148, 124, 396, 24)];
    ...
    UILabel *orderIdLabel = [[UILabel alloc] initWithFrame:CGRectMake(148, 148, 396, 24)];
    orderIdLabel.text = [NSString stringWithFormat:NSLocalizedString(@"Order ID: #%@", nil), [[Quote sharedQuote].order getId]];
    orderIdLabel.font = [UIFont systemFontOfSize:20];
    orderIdLabel.backgroundColor = [UIColor clearColor];
    [self.view addSubview:orderIdLabel];
    
    UILabel *invoiceLabel = [[UILabel alloc] initWithFrame:CGRectMake(100, 212, 396, 24)];
...
- (void)viewDidAppear:(BOOL)animated{
    if ([[[NSUserDefaults standardUserDefaults] stringForKey:@"autoprint"] boolValue]) {
        [self printReceipt];
    }
}
...
- (void)startNewOrder
{
    ...
    [CartViewController sharedInstance].isHoldOrder = NO;
    //End
