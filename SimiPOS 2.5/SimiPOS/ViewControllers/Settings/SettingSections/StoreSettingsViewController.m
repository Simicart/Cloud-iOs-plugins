...
- (void)loadStoreInfoThread
{
    ...
    if (![store isLoaded]) {
        ...
        //ravi
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didGetStoreInfo:) name:@"DidGetStoreInfo" object:nil];
        StoreInfoModel *storeInfo = [StoreInfoModel new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        if(WINDOW_WIDTH > 1024){
            storeLogo = [[UIImageView alloc] initWithFrame:CGRectMake(0, 10, 800, 54)];
        }else{
            storeLogo = [[UIImageView alloc] initWithFrame:CGRectMake(45, 10, 506, 54)];
        }
        ...
        if(WINDOW_WIDTH > 1024){
            storeAddress = [[UILabel alloc] initWithFrame:CGRectMake(0, 68, 800, 44)];
        }else{
            storeAddress = [[UILabel alloc] initWithFrame:CGRectMake(45, 68, 506, 44)];
        }
    ...
    if ([indexPath section] == 1) {
        ...
        if ([self isStoreLoaded]) {
            ...
            // Johan
            if(WINDOW_WIDTH > 1024){
                storeAddress.frame = CGRectMake(0, 68, 800, [addresses count] * 22 + 22);
            }else{
...
//Ravi
- (void)didGetStoreInfo : (NSNotification*)noti{
    [animation stopAnimating];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        ...
        Store *store = [Store currentStore];
        [store addEntriesFromDictionary:respone.data];
        store.isLoadedInfo = YES;
