...
@property (strong, nonatomic) MSNumberPadString *keyboard;
...
- (BOOL)numberPad:(MSNumberPadString *)numberPad willChangeValue:(NSInteger)value
{
    if(value == 11){
        ...
        if (numberPad.textField.text.length == keyboard.maxInput) {
            return NO;
        }
    }
    return YES;
...
-(NSString *)numberPadFormatOutput:(MSNumberPadString *)numberPad{
    if(isZero){
        ...
        return [NSString stringWithFormat:@"%@", numberPad.stringValue];
    }
}

- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"tapLockKey" object:nil];
    if (keyboard == nil) {
        keyboard = [MSNumberPadString keyboard];
...
- (BOOL)numberPadShouldDone:(MSNumberPadString *)numberPad
{
    ...
    if ([text length] == keyboard.maxInput) {
