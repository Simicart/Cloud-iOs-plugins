...
-(void)viewWillAppear:(BOOL)animated{
    ...
    // fix to test
    //    [self updateButtonClick:self.btnUpdate];
}
...
- (void)viewDidLoad {
    ...
    self.storeLabel.text = NSLocalizedString(@"Store", nil);
    self.cashDrawerLabel. text = NSLocalizedString(@"Cash Drawer", nil);
    [self.btnUpdate setTitle:NSLocalizedString(@"Submit", nil) forState:(UIControlStateNormal)];
...
- (IBAction)updateButtonClick:(id)sender {
    ...
    [params setValue:@"config.setStoreData" forKey:@"method"];
    [params setValue:[[Configuration globalConfig] valueForKey:@"session"] forKey:@"session"];
    [params setValue:storeID forKey:@"store_id"];
    [params setValue:tillID forKey:@"till_id"];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didSetStoreData:) name:@"DidSetStoreData" object:storeModel];
    [storeModel setStoreDataWithParams:params];
    //End
}

//Ravi
- (void)didSetStoreData: (NSNotification *)noti{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DidSetStoreData" object:storeModel];
    ...
    if([respone.status isEqualToString:@"SUCCESS"]){
        ...
        NSDictionary * data =[storeModel objectForKey:@"data"];
        if(data && [data isKindOfClass:[NSDictionary class]]){
            ...
            //add config price
            [Configuration globalConfig].configPrice = [[NSMutableDictionary alloc]initWithDictionary:[data objectForKey:@"price_format"]];
