...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideBackButtonWhenOrderSuccess) name:@"NofityHideBackButtonWhenOrderSuccess" object:nil];
       ...
       [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateTotalPrice) name:@"NofityUpdateTotalPrice" object:nil];
    
    
    [self.totalPriceLabel removeFromSuperview];
    self.totalPriceLabel.hidden =YES;
    self.totalPriceLabel.tag =999;
    
    [self.cartController.view addSubview:self.totalPriceLabel];
    
    [self.commentLabel removeFromSuperview];
    self.commentLabel.hidden =YES;
    [self.cartController.view addSubview:self.commentLabel];
    
}

#pragma mark - Cap nhat tong tien
-(void)updateTotalPrice{
    self.totalLabel.text = [Price format:[[Quote sharedQuote] getGrandTotalOffline]];
    
    self.totalPriceLabel.text =[NSString stringWithFormat:@"SUM TOTAL : %@", self.totalLabel.text];
    ...
    if([[Quote sharedQuote] objectForKey:@"order_comment"]){
      self.commentLabel.text =[[Quote sharedQuote] objectForKey:@"order_comment"];
    }
}
...
- (void)clearShoppingCart
{
    ...
    [[[NSThread alloc] initWithTarget:[Quote sharedQuote] selector:@selector(assignCustomerOffline:) object:nil] start];
}
...
#pragma mark - Hiển thị nội dung comment khi nhấn place order
-(void)showCommentDetail{
    if([[Quote sharedQuote] objectForKey:@"order_comment"]){
        self.commentLabel.text=[NSString  stringWithFormat:@"Order comment: %@",[[Quote sharedQuote] objectForKey:@"order_comment"]];
    }
}
