...
- (UITableViewCell *)customerViewCell:(NSIndexPath *)indexPath
{
    ...
    if ([quote hasCustomer]) {
        ...
        cell.imageView.image = [UIImage imageNamed:@"user_male.png"];
    } else {
...
//        cell.imageView.image =  [UIImage imageNamed:@"icon_white_user_male_circle.png"];
        cell.imageView.image =  [UIImage imageNamed:@"add_user"];
...
- (UITableViewCell *)productViewCell:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView setImageWithURL:[NSURL URLWithString:[item objectForKey:@"image"]] placeholderImage:[UIImage imageNamed:@"product_placeholder.png"]];
    
    //Image Custom Sale
    if([[item.product objectForKey:@"id"] isEqualToString:@"CustomSaleID"]){
        cell.imageView.image =[UIImage imageNamed:@"icon_custom_sale.png"];
    }
    //Image Discount
    else if([[item.product objectForKey:@"id"] isEqualToString:@"DiscountID"])
    {
         cell.imageView.image =[UIImage imageNamed:@"discount_icon.png"];
    }
...
- (UITableViewCell *)totalsViewCell:(NSIndexPath *)indexPath
{
    ...
    if([[total objectForKey:@"code"] isEqualToString:@"subtotal"]){
        cell.imageView.image =[UIImage imageNamed:@"icon_custom_sale.png"];
        
    }else if([[total objectForKey:@"code"] isEqualToString:@"discount"]){
        cell.imageView.image =[UIImage imageNamed:@"discount_icon.png"];
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        if ([indexPath section] == 0) {
            ...
            [[[NSThread alloc] initWithTarget:quote selector:@selector(assignCustomerOffline:) object:nil] start];
            return;
        }
        // Remove Item
        QuoteItem *item = [[quote getAllItems] objectAtIndex:[indexPath row]];
        [quote removeItemOfline:item];
