...
- (void)viewDidLoad
{
    ...
    // Center View
    UIView *centerView = [[UIView alloc] initWithFrame:CGRectMake(110, 60, 386, 520)];
    ...
    productShipping.onTintColor  = [UIColor barBackgroundColor];
    ...
    [keyboard showOn:self atFrame:CGRectMake(111, frame.origin.y + 60, frame.size.width, 265)];
...
- (void)addProductToCart:(id)sender
{
    ...
    [(ProductCollectionViewVC *)self.parentViewController customSale:self];
}
...
-(void)addToCartThread
{
    ...
    if (productName.text) {
        [[Quote sharedQuote] addCustomSaleOffline:@{@"name": productName.text, @"is_virtual": isVirtual, @"price": price}];
    } else {
        [[Quote sharedQuote] addCustomSaleOffline:@{@"is_virtual": isVirtual, @"price": price}];
    }
}
