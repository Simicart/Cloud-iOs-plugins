...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[option objectForKey:@"group"] isEqualToString:@"text"]) {
        ...
        UITextView *textView = (UITextView *)[cell.contentView viewWithTag:CELL_TEXT_FIELD_TAG];
        if(!textView){
            textView =[[UITextView alloc] init];
            textView.tag = CELL_TEXT_FIELD_TAG;
            
            textView.frame = CGRectMake(10, 30, self.tableWidth - 20, 80);
            textView.textColor = [UIColor blackColor];
            
            //Dung de lay lai gia tri nhap vao cho phan option
...
{
    if (view != nil & self.masterOptions.count >0 & self.detailControl.view.hidden) {
...
{
    ProductOptions * productOptions =(ProductOptions *)self.parentViewController;
    
//    if(groupTextTypeAreaTextView && groupTextTypeAreaTextView.text.length >0){
//        
//        NSDictionary *option = [self.masterOptions objectAtIndex:groupTextTypeAreaTextView.tag];
//        if(option && [option objectForKey:@"name"]){
//           [productOptions.productOptions setObject:groupTextTypeAreaTextView.text forKey:[option objectForKey:@"name"]];
//            
//           // groupTextTypeAreaTextView = nil;
//        }
//    }
//    
    [productOptions addProductToCart];
}
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    ProductOptions * productOptions =(ProductOptions *)self.parentViewController;
    NSDictionary *option = [self.masterOptions objectAtIndex:textView.tag];
    if(option && [option objectForKey:@"name"]){
        [productOptions.productOptions setObject:textView.text forKey:[option objectForKey:@"name"]];
    }
}
