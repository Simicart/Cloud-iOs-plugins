...
- (void)viewDidLoad
{
    ...
    if (![[Reachability reachabilityForInternetConnection] isReachable]) {
        [Utilities alert:NSLocalizedString(@"Network Error", nil) withMessage:NSLocalizedString(@"You must connect to a Wi-Fi or cellular data network to access the RetailerPOS", nil)];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    if ([MSValidator validateEmail:username.text] && password.text && ![password.text isEqualToString:@""]) {
        [self login];
        return YES;
    }
    return NO;
